...
{
    $sql = 'INSERT INTO "SUPERB_MED".users_tokens(user_id, selector, hashed_validator, expiry)
...
{
    $sql = 'DELETE FROM "SUPERB_MED"users_tokens WHERE user_id = :user_id';
    $statement = db()->prepare($sql);
    $statement->bindValue(':user_id', $user_id);
...
function find_user_by_token(string $token)
{
    ...
    $sql = 'SELECT users.id, username
            FROM "SUPERB_MED".users
            INNER JOIN "SUPERB_MED".users_tokens ON user_id = users.id
            WHERE selector = :selector AND
                expiry > now()
            LIMIT 1';
